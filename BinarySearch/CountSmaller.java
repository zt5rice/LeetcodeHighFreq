...
public class CountSmaller {
    public List<Integer> countSmaller(int[] nums) {
        ...
        getPos(pos); //
        int[] helper = new int[nums.length];// an array which is a copy of the entire pos each time after pos’s left and
                                            // right section have been sorted during the merge sort process
    ...
    public void mergeSort(int[] pos, int[] res, int[] helper, int[] nums, int left, int right) {
        ...
        int j = mid + 1;
        int k = left;
        // pos[k] is being processed

        while (i <= mid) {
            // case1: nums[helper[i]] <= nums[helper[j]]: copy helper[i] to pos and update
            // res; move i
